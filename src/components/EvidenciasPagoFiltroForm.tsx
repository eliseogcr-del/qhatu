...
export default function EvidenciasPagoFiltroForm({
  q,
  desde,
  hasta,
  hayFiltros,
  ...
  const navegar = (params: { q: string; desde: string; hasta: string }) => {
    ...
    <div className="mb-4 flex flex-wrap items-end gap-3 rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      ...
      <div>
        ...
        <input
          type="date"
          value={desde}
          onChange={(e) => navegar({ q: query, desde: e.target.value, hasta })}
      ...
      <div>
        ...
        <input
          type="date"
          value={hasta}
          onChange={(e) => navegar({ q: query, desde, hasta: e.target.value })}
          className="rounded-lg border border-gray-300 px-3 py-2 text-sm"
        />
      </div>
      {hayFiltros && (
